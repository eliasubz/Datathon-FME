...
print("Standardizing PCA embeddings...")
scaler_embeddings = StandardScaler()
embeddings_train_pca = scaler_embeddings.fit_transform(embeddings_train_pca)
embeddings_test_pca = scaler_embeddings.transform(embeddings_test_pca)
...
def build_full_preprocessing_pipeline(df):
    ...
    categorical_cols = one_hot_columns + target_encode_columns
    numeric_columns = [c for c in df.columns if c not in categorical_cols and df[c].dtype in ['int64', 'float64']]
    ...
    print(f"Standardizing {len(numeric_columns)} numeric columns: {numeric_columns}")
...
# Standardize extra features separately
print("\nStandardizing extra training features...")
scaler_extra = StandardScaler()
extra_train_features_scaled = scaler_extra.fit_transform(extra_train_features)
...
X_train_final = np.hstack([
    ...
    extra_train_features_scaled  # ← Now scaled
])
...
print(f"  PCA embeddings (scaled):      {embeddings_train_pca.shape}")
print(f"  Extra features (scaled):      {extra_train_features_scaled.shape}")
print(f"  ---")
print(f"  FINAL COMBINED SHAPE:         {X_train_final.shape}")
print(f"  Total features: {X_train_processed.shape[1]} + {embeddings_train_pca.shape[1]} + {extra_train_features_scaled.shape[1]} = {X_train_final.shape[1]}")

print("\n" + "="*70)
print("FINAL RESULTS - TEST DATA:")
print("="*70)
print(f"  Pipeline processed features:  {X_test_processed.shape}")
print(f"  PCA embeddings (scaled):      {embeddings_test_pca.shape}")
print(f"  ---")
